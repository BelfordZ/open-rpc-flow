...
export class TimeoutValidator {
  ...
  static validateTimeout(timeoutMs: unknown, defaultValue?: number): number {
    ...
    if (timeoutMs === undefined || timeoutMs === null) {
      ...
      throw new ValidationError('Timeout value is required when no default is provided', {
        value: timeoutMs,
      });
    }
    ...
    if (typeof timeoutMs !== 'number') {
      throw new ValidationError('Timeout value must be a number', {
        value: timeoutMs,
        type: typeof timeoutMs,
      });
    }

    // Check for NaN or Infinity
    if (!isFinite(timeoutMs) || isNaN(timeoutMs)) {
      throw new ValidationError('Timeout value must be a finite number', { value: timeoutMs });
    }
    ...
    if (roundedTimeout < MIN_TIMEOUT_MS) {
      throw new ValidationError(`Timeout value must be at least ${MIN_TIMEOUT_MS}ms`, {
    ...
    if (roundedTimeout > MAX_TIMEOUT_MS) {
      throw new ValidationError(`Timeout value must not exceed ${MAX_TIMEOUT_MS}ms`, {
        value: roundedTimeout,
