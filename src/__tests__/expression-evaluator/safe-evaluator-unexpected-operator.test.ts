...
describe('SafeExpressionEvaluator - Unexpected Operator Error', () => {
  ...
  it('throws when expression starts with an operator', () => {
    ...
    expect(() => evaluator.evaluate('* 2', {})).toThrow("Failed to evaluate expression: * 2. Got error: Operator * missing left operand");
    expect(() => evaluator.evaluate('/ 2', {})).toThrow("Failed to evaluate expression: / 2. Got error: Operator / missing left operand");
    expect(() => evaluator.evaluate('&& true', {})).toThrow("Failed to evaluate expression: && true. Got error: Unexpected operator");
    expect(() => evaluator.evaluate('|| true', {})).toThrow("Failed to evaluate expression: || true. Got error: Unexpected operator");
  });

  it('throws when consecutive operators are used without values between them', () => {
    // Having two operators in a row should throw
    expect(() => evaluator.evaluate('2 + * 3', {})).toThrow("Failed to evaluate expression: 2 + * 3. Got error: Operator + missing right operand");
    expect(() => evaluator.evaluate('2 * + 3', {})).toThrow("Failed to evaluate expression: 2 * + 3. Got error: Operator * missing right operand");
    expect(() => evaluator.evaluate('2 && || 3', {})).toThrow("Failed to evaluate expression: 2 && || 3. Got error: Unexpected operator");
    expect(() => evaluator.evaluate('2 == != 3', {})).toThrow("Failed to evaluate expression: 2 == != 3. Got error: Unexpected operator");
  });

  it('throws when expression ends with an operator', () => {
    // Ending an expression with an operator should throw
    expect(() => evaluator.evaluate('2 +', {})).toThrow("Failed to evaluate expression: 2 +. Got error: Operator + missing right operand"); // Could be parsed as incomplete
    expect(() => evaluator.evaluate('2 *', {})).toThrow("Failed to evaluate expression: 2 *. Got error: Operator * missing right operand");
    expect(() => evaluator.evaluate('true &&', {})).toThrow("Failed to evaluate expression: true &&. Got error: Invalid operation node");
  });
