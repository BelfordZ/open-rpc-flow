...
describe('Tokenizer Object Spread Operator Coverage', () => {
  ...
  describe('object literal spread operator (lines 557-558)', () => {
    ...
    it('tokenizes object literal with nested spread operators', () => {
      ...
      const firstSpreadIndex = objectTokens.findIndex(token => 
        token.type === 'operator' && token.value === '...'
      );
      ...
      const nestedObjectIndex = objectTokens.findIndex(token => 
      ...
      const nestedSpreadIndex = nestedObjectTokens.findIndex(token => 
        token.type === 'operator' && token.value === '...'
      );
