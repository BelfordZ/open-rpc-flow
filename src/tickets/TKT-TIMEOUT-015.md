...
export class RetryPolicy {
  ...
  constructor(options: RetryPolicyOptions = {}) {
    ...
    this.options = {
      ...
      timeoutRetry: {
        ...
        retryableStepTypes: [StepType.Request, StepType.Loop, StepType.Sequence, StepType.Parallel],
   ...
   */
  private isTimeoutRetryable(error: TimeoutError, step: Step, retryCount: number): boolean {
    ...
    if (timeoutRetry.retryableStepTypes && !timeoutRetry.retryableStepTypes.includes(step.type)) {
      return false;
    }
    ...
    while (true) {
      try {
        ...
        if (retryPolicy.isRetryable(error, step, retryCount)) {
          ...
          // Log retry attempt
          this.log(
            `Retrying request step "${step.name}" after error: ${error.message} (attempt ${retryCount})`,
          );
          ...
          if (error instanceof TimeoutError) {
            ...
            this.log(
              `Adjusted timeout for retry: ${adjustedTimeout}ms (original: ${originalTimeout}ms)`,
            );
          }
