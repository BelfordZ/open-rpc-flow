...
describe('SafeExpressionEvaluator Coverage Improvements', () => {
  ...
  describe('template literal evaluation', () => {
    it('handles template literals with mixed content types', () => {
      ...
      const result = evaluator.evaluate('`${context.value1} ${context.value2} ${context.value3}`', {});
  ...
  describe('AST evaluation for different node types', () => {
    ...
    it('evaluates object literals correctly', () => {
      ...
      context.value = 'test';
      expect(evaluator.evaluate('{ "prop": ${context.value} }', {}))
        .toEqual({ prop: 'test' });
      
      // Object with nested objects
      expect(evaluator.evaluate('{ a: 1, b: { c: 3 } }', {}))
        .toEqual({ a: 1, b: { c: 3 } });
    });
    ...
    it('evaluates array literals correctly', () => {
      ...
      context.value = 'test';
      expect(evaluator.evaluate('[${context.value}, 123]', {}))
        .toEqual(['test', 123]);
      
      // Nested arrays
      expect(evaluator.evaluate('[[1, 2], [3, 4]]', {}))
        .toEqual([[1, 2], [3, 4]]);
    });
  });
  ...
  describe('object spread operator handling', () => {
    it('spreads objects into object literals correctly', () => {
      ...
      // Spread one object
      expect(evaluator.evaluate('{ ...${context.obj1} }', {}))
        .toEqual({ a: 1, b: 2 });
      
      // Spread multiple objects
      expect(evaluator.evaluate('{ ...${context.obj1}, ...${context.obj2} }', {}))
        .toEqual({ a: 1, b: 2, c: 3, d: 4 });
      
      // Spread with additional properties
      expect(evaluator.evaluate('{ ...${context.obj1}, e: 5 }', {}))
        .toEqual({ a: 1, b: 2, e: 5 });
      
      // Override spread properties
      expect(evaluator.evaluate('{ ...${context.obj1}, a: 10 }', {}))
        .toEqual({ a: 10, b: 2 });
    });
  ...
  describe('array spread operator handling', () => {
    it('spreads arrays into array literals correctly', () => {
      ...
      // Spread one array
      expect(evaluator.evaluate('[...${context.arr1}]', {}))
        .toEqual([1, 2]);
      
      // Spread multiple arrays
      expect(evaluator.evaluate('[...${context.arr1}, ...${context.arr2}]', {}))
        .toEqual([1, 2, 3, 4]);
      
      // Spread with additional elements
      expect(evaluator.evaluate('[...${context.arr1}, 5]', {}))
        .toEqual([1, 2, 5]);
      
      // Spread at different positions
      expect(evaluator.evaluate('[0, ...${context.arr1}, 5]', {}))
        .toEqual([0, 1, 2, 5]);
    });
  ...
  describe('error handling', () => {
    ...
    it('handles different types of errors in operation evaluation', () => {
      ...
      try {
        ...
        expect(() => evaluator.evaluate('2 + 3', {})).toThrow('Failed to evaluate operation: Custom error message');
        ...
        expect(() => evaluator.evaluate('2 + 3', {})).toThrow(ExpressionError);
        expect(() => evaluator.evaluate('2 + 3', {})).toThrow('Failed to evaluate operation: Unknown error');
      } finally {
  ...
  describe('parsing complex expressions', () => {
    ...
    it('handles mismatched parentheses', () => {
      ...
      expect(() => evaluator.evaluate('2 + 3)', {})).toThrow(ExpressionError);
      expect(() => evaluator.evaluate('2 + 3)', {})).toThrow('Failed to evaluate expression: 2 + 3). Got error: Mismatched parentheses');
    });
  ...
  describe('timeout handling', () => {
    it('respects the timeout when evaluating expressions', () => {
      ...
      expect(() => evaluator['checkTimeout'](Date.now() - 2000)).toThrow('Expression evaluation timed out');
    });
  });
