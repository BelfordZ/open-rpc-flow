...
export interface Token {
  type: 'string' | 'number' | 'operator' | 'reference' | 'object_literal' | 'array_literal' | 'punctuation' | 'identifier' | 'key' | 'template_literal';
...
function isUnaryContext(state: TokenizerState): boolean {
  ...
  return lastType === 'operator' || 
         (state.tokens[state.tokens.length - 1].type === 'punctuation' && state.tokens[state.tokens.length - 1].value === ',');
}
...
function validateChar(char: string): void {
  if (!isValidChar(char)) {
    throw new TokenizerError(`Invalid character in text: ${char}`);
  }
}
...
function findPrevNonWhitespace(expression: string, startIndex: number): string | null {
...
function validateOperandCount(state: TokenizerState, operator: string): void {
  if (state.tokens.length === 0) {
...
function handleSpreadOperator(state: TokenizerState): Token {
...
function flushTextBuffer(state: TokenizerState, char: string, logger: Logger): void {
...
function handleContainer(state: TokenizerState, char: string): void {
    ...
    if (isClosing) {
        ...
        if ((openChar === '{' && char !== '}') || (openChar === '[' && char !== ']')) {
            throw new TokenizerError(`Mismatched container: expected ${openChar === '{' ? '}' : ']'} but found ${char}`);
        }
        ...
        if (containerType === 'object_literal') {
            for (let i = 0; i < containerTokens.length; i++) {
                if (containerTokens[i].type === 'identifier' && 
        ...
        state.currentContainer = state.containerStack.length > 0 ? state.containerStack[state.containerStack.length - 1] : null;
    } else {
        flushTextBuffer(state, char, state.logger);
...
function handleOperator(state: TokenizerState): Token {
    ...
    if (state.currentIndex + 2 < state.expression.length) {
        const potentialOperator3 = state.expression.substring(state.currentIndex, state.currentIndex + 3);
    ...
    const nextChar = state.currentIndex + 1 < state.expression.length ? state.expression[state.currentIndex + 1] : '';
    if (nextChar) {
...
function handleTemplateLiteral(state: TokenizerState): Token[] {
    ...
    while (state.currentIndex < state.expression.length) {
        ...
        if (char === '$' && state.expression[state.currentIndex + 1] === '{') {
            ...
            const expressionContent = state.expression.substring(expressionStartIndex, state.currentIndex - 1);
...
interface ObjectLiteralToken extends Token {
    type: 'object_literal';
    value: Token[];
}
...
function tokenizeExpression(expression: string, logger: Logger): Token[] {
    const state: TokenizerState = createTokenizerState(expression, logger);
    ...
    while (state.currentIndex < expression.length) {
        ...
        if (char === '"' || char === "'") {
            if (state.textBuffer) {
                state.tokens.push({
                    ...
                    value: isNumber(state.textBuffer.trim()) ? Number(state.textBuffer.trim()) : (inBraces ? state.textBuffer : state.textBuffer.trim()),
                    raw: state.textBuffer
                });
                state.textBuffer = '';
            }
        ...
        if ((state.currentIndex + 1 < expression.length && expression.substring(state.currentIndex, state.currentIndex + 2) === '??') 
            || isOperator(char) || isPunctuation(char)) {
            if (state.textBuffer) {
                state.tokens.push({
                    type: isNumber(state.textBuffer.trim()) ? 'number' : (inBraces ? 'string' : 'identifier'),
                    value: isNumber(state.textBuffer.trim()) ? Number(state.textBuffer.trim()) : (inBraces ? state.textBuffer : state.textBuffer.trim()),
        ...
        if (isWhitespace(char)) {
            if (inBraces) {
                if (!inTextSequence) {
                    if (state.textBuffer) {
                        state.tokens.push({
                            ...
                            value: isNumber(state.textBuffer.trim()) ? Number(state.textBuffer.trim()) : state.textBuffer,
                            raw: state.textBuffer
                        });
                        state.textBuffer = '';
                    }
                ...
                state.tokens.push({
                    ...
                    value: isNumber(state.textBuffer.trim()) ? Number(state.textBuffer.trim()) : state.textBuffer.trim(),
                    raw: state.textBuffer
                });
                state.textBuffer = '';
            }
...
function createToken(type: Token['type'], value: string, raw: string): Token {
  return {
    type,
    value,
    raw
...
function handleIdentifier(state: TokenizerState): Token {
    let identifier = '';
    const startIndex = state.currentIndex;
...
function handlePunctuation(state: TokenizerState): Token {
    const char = state.expression[state.currentIndex];
    state.currentIndex++;
    return {
        type: 'punctuation',
