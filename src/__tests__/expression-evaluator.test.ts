...
describe('ExpressionEvaluator', () => {
  ...
  beforeEach(() => {
    ...
    evaluator = new ExpressionEvaluator(referenceResolver, context);
  });
  ...
  describe('evaluateExpression', () => {
    ...
    test('evaluates complex object literals with references', () => {
      const result = evaluator.evaluateExpression('{ id: ${step1.data.value}, config: ${context.config.threshold} }');
      expect(result).toEqual({ id: 42, config: 100 });
    });

    test('throws on invalid expressions', () => {
      expect(() => evaluator.evaluateExpression('invalid syntax')).toThrow('Failed to evaluate expression');
    });

    it('throws on undefined references', () => {
      expect(() => evaluator.evaluateExpression('${nonexistent.value}')).toThrow("Reference 'nonexistent' not found. Available references are: step1, context");
    });
    ...
    describe('wrapped step results', () => {
      ...
      beforeEach(() => {
        ...
        referenceResolver = new ReferenceResolver(new Map([['userInfo', wrappedResult]]), {});
        evaluator = new ExpressionEvaluator(referenceResolver, context);
      });
      ...
      it('should not allow direct property access that skips result', () => {
        ...
        expect(() => evaluator.evaluateExpression('${userInfo.name}', {}))
          .toThrow('Cannot access property');
        expect(() => evaluator.evaluateExpression('${userInfo.age}', {}))
          .toThrow('Cannot access property');
      });
      ...
      it('should handle nested step results correctly', () => {
        ...
        referenceResolver = new ReferenceResolver(new Map([['nested', nestedResult]]), {});
        evaluator = new ExpressionEvaluator(referenceResolver, context);
        ...
        expect(() => evaluator.evaluateExpression('${nested.user.result.name}', {}))
          .toThrow('Cannot access property');
        expect(() => evaluator.evaluateExpression('${nested.result.user.name}', {}))
          .toThrow('Cannot access property');
      });
      ...
      it('should handle array results correctly', () => {
        ...
        referenceResolver = new ReferenceResolver(new Map([['users', arrayResult]]), {});
        evaluator = new ExpressionEvaluator(referenceResolver, context);
        ...
        expect(() => evaluator.evaluateExpression('${users[0].name}', {}))
          .toThrow();
      });
      ...
      it('should handle array results with both notations', () => {
        ...
        expect(() => evaluator.evaluateExpression('${users[0].name}', {}))
          .toThrow('Cannot access property');
        expect(() => evaluator.evaluateExpression('${users["result"][0].name}', {}))
          .toThrow('Cannot access property');
      });
      ...
      it('should handle array notation edge cases', () => {
        ...
        referenceResolver = new ReferenceResolver(new Map([['complex', complexResult]]), {});
        evaluator = new ExpressionEvaluator(referenceResolver, context);

        // Special characters in keys require array notation
        expect(evaluator.evaluateExpression('${complex.result["special.key"]}', {})).toBe('special value');
        expect(evaluator.evaluateExpression('${complex.result["key-with-dash"]}', {})).toBe('dash value');
        ...
        // These should fail
        expect(() => evaluator.evaluateExpression('${complex.result.special.key}', {}))
          .toThrow(); // Can't use dot notation with special characters
        expect(() => evaluator.evaluateExpression('${complex.result.0}', {}))
          .toThrow(); // Can't use dot notation with numeric keys
      });
      ...
      it('should handle array notation with expressions', () => {
        ...
        referenceResolver = new ReferenceResolver(new Map([['arr', arrayResult]]), {});
        evaluator = new ExpressionEvaluator(referenceResolver, context);
        ...
        // These should fail
        expect(() => evaluator.evaluateExpression('${arr.result[-1]}', {}))
          .toThrow(); // Invalid index
        expect(() => evaluator.evaluateExpression('${arr.result[3]}', {}))
      ...
      it('should handle deeply nested array and object access', () => {
        ...
        referenceResolver = new ReferenceResolver(new Map([['deep', deepResult]]), {});
        evaluator = new ExpressionEvaluator(referenceResolver, context);

        // Deep access with mixed notation
        expect(evaluator.evaluateExpression('${deep.result.users[0].result.friends[0].result.name}', {}))
          .toBe('Charlie');
        expect(evaluator.evaluateExpression('${deep["result"]["users"][0]["result"]["friends"][1]["result"]["name"]}', {}))
          .toBe('David');
        
        // These should fail
        expect(() => evaluator.evaluateExpression('${deep.result.users[0].friends[0].name}', {}))
      ...
      it('should handle array notation with string expressions', () => {
        ...
        referenceResolver = new ReferenceResolver(new Map([['data', result]]), {});
        evaluator = new ExpressionEvaluator(referenceResolver, context);
        ...
        // These should fail
        expect(() => evaluator.evaluateExpression('${data.result[keys[0]]}', {}))
          .toThrow(); // Undefined variable
        expect(() => evaluator.evaluateExpression('${data.result[undefined]}', {}))
      ...
      it('should not treat expressions with multiple references as a single reference', () => {
        ...
        expect(() => evaluator.evaluateExpression('${foo} > ${bar}', extraContext))
          .not.toThrow('Failed to resolve reference: foo} > ${bar');
      });
    });
  });
}); 